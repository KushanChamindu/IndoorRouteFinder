...
public class Trilateration {
    ...
    private static double B(double a) {
        return a / 900000;
    }

    private static double[] C(double a, double b, double c, double d) {
        return new double[]{a * Math.cos((Math.PI / 180) * d) - b * Math.sin((Math.PI / 180) * d), a * Math.sin((Math.PI / 180) * d) + b * Math.cos((Math.PI / 180) * d), c};
    }
    ...
    private static double E(double a, double b, double c, double d) {
        double e = Math.PI, f = e * a / 180, g = e * c / 180, h = b - d, i = e * h / 180, j = Math.sin(f) * Math.sin(g) + Math.cos(f) * Math.cos(g) * Math.cos(i);
        if (j > 1) {
            j = 1;
        }
        j = Math.acos(j);
